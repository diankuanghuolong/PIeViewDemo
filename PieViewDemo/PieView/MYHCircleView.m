                 ...
                 angele:(CGFloat)angele
{
    ...
    CGFloat textStartX;
    CGFloat textStartY;
    lineLosePointX = smallCircleCenterPointX+20.0*cos(angele);
    lineLosePointY = smallCircleCenterPointY + 20.0*sin(angele);
    //    NSLog(@"%lf  %lf %lf",10.0*cos(angele),10.0*sin(angele),angele);
    NSString * OldNumberStr = [NSString stringWithFormat:@"%@",[self getNumStr:_dataArray[n][@"number"]]];
    NSString * numberStr;
    ...
    if (smallCircleCenterPointX > self.frame.size.width / 2) {//往右跑
        ...
        textStartY = lineEndPointY - 8;
        //数字
        numberStartX = textStartX + textSizeNumber.width;
        numberStartY = lineEndPointY - 8;
       ...
       /* //数字
        ...
        lineEndPointY = lineLosePointY;
        //文字
        textStartX = lineEndPointX - textSizeNumber.width;
        textStartY = lineEndPointY - 8;
        //数字
        numberStartX = textStartX - itemSizeNumber.width;
        numberStartY = lineEndPointY - 8;
        /*
         //指引线的终点
         lineEndPointX = lineLosePointX - lineWidth; //
...
//    UIBezierPath *arcPath = [UIBezierPath bezierPathWithArcCenter:CGPointMake(smallCircleCenterPointX, smallCircleCenterPointY) radius:4 startAngle:0 endAngle:PI * 2 clockwise:YES];
     UIBezierPath *arcPath = [UIBezierPath bezierPathWithArcCenter:CGPointMake(lineEndPointX, lineEndPointY) radius:2 startAngle:0 endAngle:PI * 2 clockwise:YES];
    [color set];
    [arcPath fill];
    [arcPath stroke];
    //画指引线
    ...
    if (textStartX>=self.frame.size.width/2) {//往右边跑
        [_dataArray[n][@"name"] drawInRect:CGRectMake(textStartX+5, textStartY, textSizeNumber.width, 50) withAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:12.0],NSForegroundColorAttributeName:textColor,NSParagraphStyleAttributeName:paragraph}];
    }else{
        [_dataArray[n][@"name"] drawInRect:CGRectMake(textStartX-5, textStartY, textSizeNumber.width, 50) withAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:12.0],NSForegroundColorAttributeName:textColor,NSParagraphStyleAttributeName:paragraph}];
    }
}
...
{
    numStr = [[NSString alloc] initWithFormat:@"%.0f%%",[numStr floatValue]*100];
    return numStr;
}
@end
